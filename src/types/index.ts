...
export interface AnalyticsData {
  kpis: {
    ...
    monthlyPendingRent: number;
    monthlyExpenses: number;
    netOperatingIncome: number;
  };
  monthlyTrends: Array<{ month: string; expected: number; collected: number; expenses: number }>;
  occupancyTrends: Array<{ month: string; rate: number }>;
  maintenanceByStatus: Array<{ name: string; count: number; color: string }>;
  expenseBreakdown: Array<{ name: string; amount: number }>;
  propertyPerformance: Array<{ property: string; fullName: string; revenue: number; expenses: number; occupancy: number }>;
}
